<template>
    <v-card class="mx-auto test-width">
        ...
        <v-navigation-drawer v-model="leftDrawer" absolute temporary clipped>
            <LeftDrawer/>
        </v-navigation-drawer>
    
        <v-navigation-drawer v-model="rightDrawer" right absolute temporary>
            <RightDrawer/>
        </v-navigation-drawer>
        ...
        <v-container fluid>
            <v-row dense v-if="listType == 'grid'">
                <v-col v-for="(card, index) in cards" :key="index" :cols="6" xs="6" sm="4" md="3" lg="3" xl="3">
                    <v-card>
                        <v-img :src="baseUrl+card['thumbnail-iamge']" class="white--text align-end" gradient="to bottom, rgba(0,0,0,.1), rgba(0,0,0,.5)">
